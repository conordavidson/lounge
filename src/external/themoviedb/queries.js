...
export const TMDB_fetchMovies = query => {
  const urlYearBeginParam = () =>
    query.years.min ? `&primary_release_date.gte=${query.years.min}-01-01` : ``
  const urlYearEndParam = () =>
    query.years.max ? `&primary_release_date.lte=${query.years.max}-01-01` : ``
  const urlGenreParam = () => (query.genre ? `&with_genres=${Genres[query.genre]}` : ``)
...
${urlGenreParam()}${urlYearBeginParam()}${urlYearEndParam()}`
  ...
  return fetch(
    `${urlBaseSegment}discover/movie?${urlKeyParam}${urlSettingParams}`
  )
}

export const TMDB_fetchDetails = query => {
  return fetch(
    `${urlBaseSegment}movie/${
      query.id
    }?${urlKeyParam}&append_to_response=videos,credits`
  )
}
